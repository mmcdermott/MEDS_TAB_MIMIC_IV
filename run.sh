...
do
  ...
  meds-tab-xgboost \
      ...
      task_name=$TASK \
      tabularization.min_code_inclusion_frequency="$MIN_CODE_FREQ" do_overwrite=False \
      hydra.sweeper.direction=maximize model_dir="$MEDS_TAB_COHORT_DIR/models/xgboost_window_agg_search/$TASK/"
  ...
  meds-tab-xgboost \
      ...
      task_name=$TASK \
      tabularization.min_code_inclusion_frequency="$MIN_CODE_FREQ" "$WINDOW_SIZES" do_overwrite=False "$AGGS" \
      hydra.sweeper.direction=maximize model_dir="$MEDS_TAB_COHORT_DIR/models/xgboost_all/$TASK/"
